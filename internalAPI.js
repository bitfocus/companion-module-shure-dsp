...
class instance_api {
	...
	constructor(instance) {
		...
		//global     [MODEL,SERIAL_NUM,FW_VER,DEVICE_ID,DEVICE_AUDIO_MUTE,PRESET,NA_DEVICE_NAME]
		//global-imr [IP_ADDR_NET_AUDIO_PRIMARY,IP_SUBNET_NET_AUDIO_PRIMARY,IP_GATEWAY_NET_AUDIO_PRIMARY,FLASH,
		//           PRESETX,REBOOT,LAST_ERROR_EVENT]

		//p300       [METER_RATE_IN,METER_RATE_OUT,METER_RATE_PROC,METER_RATE_ERLE,METER_RATE_AGC,CONTROL_MAC_ADDR,
		//           DEFAULT_SETTINGS,ENCRYPTION,INPUT_METER_MODE,OUTPUT_METER_MODE,USB_CONNECT]
		this.device  = {
			model:              '',           // (MXA|P300|IMR|ANI) G {32}
			serialNumber:       '',           // (MXA|P300|IMR|ANI) G {32}
			firmwareVersion:    '',           // (MXA|P300|IMR|ANI) G {18}
			deviceId:           '',           // (MXA|P300|IMR|ANI) G {31}
			audioMute:          'OFF',        // (MXA|P300|IMR|ANI) GS ON|OFF|[TOGGLE]
			preset:             0,            // (MXA|P300|IMR|ANI) GS 01-10,<0=none>
			networkAudioName:   '',           // (MXA|P300|IMR|ANI) G {31}
			ipAddressAudio:     '',           // (MXA|P300|ANI) G {15}
			ipSubnetAudio:      '',           // (MXA|P300|ANI) G {15}
			ipGatewayAudio:     '',           // (MXA|P300|ANI) G {15}
			ipMACAddress:       '',           // (MXA|P300) G {17}
			flash:              'OFF',        // (MXA|P300|ANI) GS ON|OFF
			lastErrorEvent:     '',           // (MXA|P300|ANI) G {128}
			encryption:         'OFF',        // (MXA|P300|ANI[!USB]) G ON|OFF
			inputMeterMode:     'POST_FADER', // (P300|ANI) GS PRE_FADER|PRE_FADER
			outputMeterMode:    'POST_FADER', // (P300|ANI) GS PRE_FADER|PRE_FADER
			usbConnect:         'OFF',        // (P300|ANIUSB) G ON|OFF|ERROR

			meterRate:          0,            // (MXA|ANI) GS 0|100-99999
			meterRateMixerGain: 0,            // (MXA) GS 0|100-99999
			meterRatePostGate:  0,            // (MXA910) GS 0|100-99999
			meterRatePreComp:   0,            // (MXA910) GS 0|100-99999
			meterRateAECRef:    0,            // (MXA910) GS 0|100-99999
			meterRateIn:        0,            // (P300) GS 0|100-99999
			meterRateOut:       0,            // (P300) GS 0|100-99999
			meterRateProc:      0,            // (P300) GS 0|100-99999
			meterRateERLE:      0,            // (P300) GS 0|100-99999
			meterRateAGC:       0,            // (P300) GS 0|100-99999

			presets: {                        // (MXA|P300|ANI) G {25}
				1:  'Preset 1',
				2:  'Preset 2',
				3:  'Preset 3',
				4:  'Preset 4',
	...
	/**
	 * Returns the device state object.
	 *
